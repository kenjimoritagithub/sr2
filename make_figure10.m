...
save data_figure10Ca totalRset
heatmap_performance(mean(totalRset,3),length(a_posiprop_set),[],'Figure10Ca');
...
save data_figure10Cb totalRset
heatmap_performance(mean(totalRset,3),length(a_posiprop_set),[],'Figure10Cb');
...
tmp_letters = 'EFG';
for k_model = 1:3
    heatmap_performance(mean(totalRset{k_model},3),length(a_posiprop_set),[],['Figure10' tmp_letters(k_model) 'top']);
end
% bottom graphs
tmp_plot_max = 0;
for k_model = 1:3
